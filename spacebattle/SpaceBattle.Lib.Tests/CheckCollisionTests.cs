    ...
    public class CheckCollisionTests
    {
        public CheckCollisionTests()
        {
            ...
            IoC.Resolve<Hwdtech.ICommand>("IoC.Register", "Game.UObject.FindVariations", (object[] args) =>
            {
                ...
                var variation = positionFirst.Select((value, index) => value - positionSecond[index]).Concat(velocityFirst.Select((value, index) => value - velocitySecond[index]).ToList()).ToList();
